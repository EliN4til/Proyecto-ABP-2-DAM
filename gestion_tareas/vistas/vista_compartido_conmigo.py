...
def VistaCompartidoConmigo(page: ft.Page):
    ...
    FILTROS_ORDEN = ["Más reciente primero", "Más antiguo primero", "Alfabético A-Z", "Alfabético Z-A"]
    ...
    def mostrar_dialog_filtros(e):
        #radio buttons para tags
        radio_tags = ft.RadioGroup(
            value=filtro_tag_actual[0],
            content=ft.Column(
                controls=[ft.Radio(value=tag, label=tag) for tag in FILTROS_TAGS],
                spacing=2,
            ),
        )

        #radio buttons para orden
        radio_orden = ft.RadioGroup(
            value=filtro_orden_actual[0],
            content=ft.Column(
                controls=[ft.Radio(value=orden, label=orden) for orden in FILTROS_ORDEN],
                spacing=2,
            ),
        )
        ...
        radio_estado = ft.RadioGroup(
            ...
            content=ft.Column(
                controls=[ft.Radio(value=estado, label=estado) for estado in FILTROS_ESTADO],
                spacing=2,
            ),
        )
        ...
        def aplicar_filtros(e):
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text(f"Filtros aplicados: {filtro_tag_actual[0]}, {filtro_orden_actual[0]}, {filtro_estado_actual[0]}")
            )
            page.snack_bar.open = True
            page.update()
        ...
        dialog_filtros = ft.AlertDialog(
            ...
            content=ft.Container(
                ...
                content=ft.ListView(
                    controls=[
                        ft.Text("Por Tag:", size=13, weight=ft.FontWeight.BOLD, color=COLOR_LABEL),
                        radio_tags,
